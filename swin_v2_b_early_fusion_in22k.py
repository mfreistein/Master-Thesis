...
class EnsembleModel(nn.Module):
    def __init__(self, original_model):
        ...
        self.initial_conv = nn.Conv2d(6, 3, kernel_size=(1, 1), stride=(1, 1))
        init.kaiming_normal_(self.initial_conv.weight, mode='fan_out', nonlinearity='relu')
        self.swin = original_model
        self.swin.classifier = nn.Linear(in_features=1024, out_features=102, bias=True)

    def forward(self, pixel_values_rgb, pixel_values_depth):
        rgbd_tensor = torch.cat((pixel_values_rgb, pixel_values_depth), dim=1)
        x = self.initial_conv(rgbd_tensor)
        out = self.swin(x)
        return out.logits

##Hagberg et al. alternative 
#class EnsembleModel(nn.Module):
#    def __init__(self, original_model):
#        super(EnsembleModel, self).__init__()
#
#        self.initial_conv = nn.Conv2d(1, 3, kernel_size=(1, 1), stride=(1, 1))
#        init.kaiming_normal_(self.initial_conv.weight, mode='fan_out', nonlinearity='relu')
#        self.swin = original_model
#        self.swin.classifier = nn.Linear(in_features=1024, out_features=102, bias=True)
#
#    def forward(self, pixel_values_rgb, pixel_values_depth):
#        pixel_values_depth = pixel_values_depth[:, 0, :, :].unsqueeze(1)
#        pixel_values_depth = self.initial_conv(pixel_values_depth)
#        merged_rgbd = pixel_values_rgb + pixel_values_depth
#        out = self.swin(merged_rgbd)
#        return out.logits
...
def generate_datasets(data, train_scheme, transform_train):
    ...
    if data == "ZoeDepth_Metric":
        depth_folder_base = '/mnt/metric-data/'
        processor = None
        if train_scheme == "CH":
        ...
        depth_folder_base = '/mnt/data-target-val/data_depth/'
    ...
    train_dataset = CustomImageDataset(rgb_folder_path=rgb_folder_base + 'train', depth_folder_path=depth_folder_base + 'train', processor=processor, transform=transform_train)
    val_dataset = CustomImageDataset(rgb_folder_path=rgb_folder_base + 'val', depth_folder_path=depth_folder_base + 'val', processor=processor, transform=transform_val)
    ...
    test_dataset = CustomImageDataset(rgb_folder_path=rgb_folder_base + 'test', depth_folder_path=depth_folder_base + 'test', processor=processor, transform=transform_val)
...
def unfreeze_classifier(model):
    for param in model.swin.classifier.parameters():
        param.requires_grad = True
    return model

def unfreeze_global_average_pooling_layer(model):
    for param in model.swin.swinv2.layernorm.parameters():
        param.requires_grad = True
    return model

def unfreeze_stage_3(model):
    stage_rgb = model.swin.swinv2.encoder.layers[3]
    for param in stage_rgb.parameters():
        param.requires_grad = True
    return model

def unfreeze_stage_2(model):
    stage_rgb = model.swin.swinv2.encoder.layers[2]
    for param in stage_rgb.parameters():
        param.requires_grad = True
    return model

def unfreeze_stage_1(model):
    stage_rgb = model.swin.swinv2.encoder.layers[1]
    for param in stage_rgb.parameters():
        param.requires_grad = True
    return model

def unfreeze_stage_0(model):
    stage_rgb = model.swin.swinv2.encoder.layers[0]
    for param in stage_rgb.parameters():
        param.requires_grad = True
    return model

def unfreeze_patch_partition_and_linear_embedding_layers(model):
    for param in model.swin.swinv2.embeddings.parameters():
        param.requires_grad = True
    return model

def unfreeze_initial_convolutional_layer(model):
...
unfreeze_layer_config_functions = {
    ...
    'initial convolutional layer': unfreeze_initial_convolutional_layer, 
    }
...
def load_ensemble_model(save_path=None):
    base_model_swin = AutoModelForImageClassification.from_pretrained("microsoft/swinv2-base-patch4-window12-192-22k", num_labels=102, ignore_mismatched_sizes=True)
    ensemble_model = EnsembleModel(base_model_swin)
    if save_path:
        model_save_dict = torch.load(save_path)
        ensemble_model.load_state_dict(model_save_dict['model_state_dict'])
...
def run_ch_training(data="ZodeDepth_Normalized", processor=None, transform_train=None, transform_val=None, batch_size=32):
    ...
    base_model_swin = AutoModelForImageClassification.from_pretrained("microsoft/swinv2-base-patch4-window12-192-22k", num_labels=102, ignore_mismatched_sizes=True)
    swinv2_early_fusion_model_ch_training = EnsembleModel(base_model_swin)

    swinv2_early_fusion_model_ch_training = freeze_all_layers(swinv2_early_fusion_model_ch_training)
    swinv2_early_fusion_model_ch_training = unfreeze_classifier(swinv2_early_fusion_model_ch_training)
    swinv2_early_fusion_model_ch_training = swinv2_early_fusion_model_ch_training.to(DEVICE)

    num_epochs_ch, loss_function_ch, optimizer_ch, scheduler_ch = get_hyperparameters_ch(swinv2_early_fusion_model_ch_training)

    print_hyperparameters(num_epochs_ch, loss_function_ch, optimizer_ch, scheduler_ch)

    swinv2_early_fusion_model_ch_training_results = training(model=swinv2_early_fusion_model_ch_training,
                                         model_name='swinv2_early_fusion_model_ch',
    ...
    swinv2_early_fusion_model_ch, train_loss_array, train_acc_array, val_loss_array, val_acc_array = swinv2_early_fusion_model_ch_training_results
    save_training_progress_data(train_loss_array, train_acc_array, val_loss_array, val_acc_array, training_run_title)
    all_outputs, all_labels, test_accuracy, balanced_acc, f1_macro = test_model(swinv2_early_fusion_model_ch, test_loader)
    print_train_test_results(val_loss_array, val_acc_array, test_accuracy, balanced_acc, f1_macro, "CH")

    return swinv2_early_fusion_model_ch
...
def run_ch_ft_training_with_config(config):
    ...
    for phase, layers_unfrozen in enumerate(unfreeze_layers):
        ...
        swinv2_early_fusion_model_ch_ft_training_results = training(model=ft_model_training,
                                         model_name='swinv2_early_fusion_model_ch_ft',
        ...
        ft_model_training, train_loss_array, train_acc_array, val_loss_array, val_acc_array = swinv2_early_fusion_model_ch_ft_training_results
...
ch_data = "ZoeDepth_Normalized"
#ch_data = "ZoeDepth_Metric"
custom_transforms = CustomTransforms()
transform_train = custom_transforms.synchronized_transform_train_ch_metric if ch_data == "ZoeDepth_Metric" else custom_transforms.synchronized_transform_train_ch
transform_val = custom_transforms.synchronized_transform_val_metric if ch_data == "ZoeDepth_Metric" else custom_transforms.synchronized_transform_val
swinv2_early_fusion_model_ch_synchronized_tranforms = run_ch_training(data=ch_data,
                                                                    processor=None,
                                                                    transform_train=transform_train,
                                                                    transform_val=transform_val,
                                                                    batch_size=32)
model_name = "CustomTransforms Metric Data" if ch_data == "ZoeDepth_Metric" else "CustomTransforms Normalized Data"
swinv2_early_fusion_model_ch_synchronized_tranforms.model_name = model_name
model_path = f"/mnt/models/swinv2_early_fusion_model_ch_customtranforms_{'metric' if ch_data == 'ZoeDepth_Metric' else 'normalized'}_depth.pt"
save_ensemble_model(swinv2_early_fusion_model_ch_synchronized_tranforms, model_path)

if ch_data != "ZoeDepth_Metric":
    swinv2_early_fusion_model_ch_huggingface_processor = run_ch_training(data=ch_data,
                                                                    processor=AutoImageProcessor.from_pretrained("microsoft/swinv2-base-patch4-window12-192-22k"),
                                                                    transform_train=None,
                                                                    transform_val=None,
                                                                    batch_size=32)
    swinv2_early_fusion_model_ch_huggingface_processor.model_name = "AutoImageProcessor"
    save_ensemble_model(swinv2_early_fusion_model_ch_huggingface_processor, "/mnt/models/swinv2_early_fusion_model_ch_autoimageprocessor_normalized_depth.pt")

################################## Full Tuning (GridSearch) ###################################

#data_options = ["ZoeDepth_Normalized", 
#                "ZoeDepth_Metric"]
#
#ch_model_options = [None,
#                    "/mnt/models/swinv2_early_fusion_model_ch_autoimageprocessor_normalized_depth.pt",
#                    "/mnt/models/swinv2_early_fusion_model_ch_customtranforms_normalized_depth.pt",
#                    "/mnt/models/swinv2_early_fusion_model_ch_customtranforms_metric_depth.pt"]
#
#transform_options_normalized_depth_data = [None,
#                                           CustomTransforms().synchronized_transform_train_ft]
#unfreeze_layers_options = [ 
#    
#    [['classifier', 'unfreeze stage 3', 'unfreeze stage 2', 'unfreeze stage 1', 'unfreeze stage 0']],
#
#    [['classifier', 'global average pooling', 'unfreeze stage 3', 'unfreeze stage 2', 'unfreeze stage 1', 'unfreeze stage 0', 'patch partition and linear embedding', 'initial convolutional layer']],
#
#    [['classifier', 'unfreeze stage 3','unfreeze stage 2'], 
#     ['classifier', 'unfreeze stage 1', 'unfreeze stage 0',]],
#
#    [['classifier', 'global average pooling', 'unfreeze stage 3','unfreeze stage 2', 'initial convolutional layer'], 
#     ['classifier', 'unfreeze stage 1', 'unfreeze stage 0', 'patch partition and linear embedding', 'initial convolutional layer']],
#    
...
#
#    [['classifier', 'global average pooling', 'unfreeze stage 3', 'patch partition and linear embedding', 'initial convolutional layer'],
#     ['classifier', 'global average pooling', 'unfreeze stage 2', 'patch partition and linear embedding', 'initial convolutional layer'],
#     ['classifier', 'global average pooling', 'unfreeze stage 1', 'patch partition and linear embedding', 'initial convolutional layer'],
#     ['classifier', 'global average pooling', 'unfreeze stage 0', 'patch partition and linear embedding', 'initial convolutional layer']],
#
#    [['classifier', 'unfreeze stage 3',],
#     ['classifier', 'unfreeze stage 3', 'unfreeze stage 2',],
...
#
#    [['classifier', 'global average pooling', 'unfreeze stage 3', 'patch partition and linear embedding', 'initial convolutional layer'],
#    ['classifier', 'global average pooling', 'unfreeze stage 3', 'unfreeze stage 2', 'patch partition and linear embedding', 'initial convolutional layer'],
#    ['classifier', 'global average pooling', 'unfreeze stage 3', 'unfreeze stage 2', 'unfreeze stage 1', 'patch partition and linear embedding', 'initial convolutional layer'],
#    ['classifier', 'global average pooling', 'unfreeze stage 3', 'unfreeze stage 2', 'unfreeze stage 1', 'unfreeze stage 0', 'patch partition and linear embedding', 'initial convolutional layer'],],
#
#]

data_options = ["ZodeDepth_Normalized"]

ch_model_options = ["/mnt/models/swinv2_early_fusion_model_ch_autoimageprocessor_normalized_depth.pt",
                    "/mnt/models/swinv2_early_fusion_model_ch_customtranforms_normalized_depth.pt",]

transform_options = [CustomTransforms().synchronized_transform_train_ft]

unfreeze_layers_options = [ 

    [['classifier', 'unfreeze stage 2', 'unfreeze stage 0']],

]

batch_size_options = [32]

all_configs = list(itertools.product(data_options, ch_model_options, transform_options, unfreeze_layers_options, batch_size_options))
